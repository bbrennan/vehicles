...
The owner initially authorized a PDF-only GitHub Release ZIP, then replaced that delivery with a direct-from-Toyota local downloader and requested removal of the hosted ZIP. The ZIP never entered Git history. Only source URLs/checksums are checked in; PDFs, extracted JSON/text/layout and local acquisition manifests are not uploaded. The same owner-supplied Toyota ToS assumption applies, not an independently verified license or a new license to downstream users. Historical manifests and publication flags remain unchanged; downloading does not approve curated facts for shopper use.
